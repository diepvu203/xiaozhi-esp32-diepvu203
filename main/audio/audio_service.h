...
public:
    ...
    // Output sample rate of the underlying codec (used by MusicPlayer to
    // resample before pushing PCM). Returns 24000 if the codec is unknown.
    int GetOutputSampleRate() const;
    // Feed raw PCM (already at the codec output sample rate, mono) directly
    // into the playback queue so AudioOutputTask writes it to the speaker.
    // Blocks (up to ~2 s) while the queue is full to pace the producer;
    // returns false if the chunk had to be dropped.
    // max_queue_depth: how many chunks may sit in the queue. Music passes a
    // larger value (e.g. 20) for a deeper jitter buffer; TTS keeps the
    // default 8 for low latency.
    bool PushPcmToPlaybackQueue(std::vector<int16_t>&& pcm, int max_queue_depth = 8);

    // While true, the Opus/TTS downlink audio is muted so it does not overlap
    // with locally-decoded music (which still reaches the speaker through
    // PushPcmToPlaybackQueue). Set true when music starts, false when it stops.
