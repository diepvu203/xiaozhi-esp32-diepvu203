...
void AudioService::AudioInputTask() {
    ...
    while (true) {
        ...
        // While music is streaming, silence the microphone so the wake-word
        // engine cannot interrupt playback. Disable the codec input to save
        // power; ReadAudioData will re-enable it when music_playing_ goes
        // back to false.
...
void AudioService::AudioOutputTask() {
    ...
    while (true) {
        ...
        // Underrun detection: if the queue was empty when we started waiting,
        // the DMA ran dry and the speaker played silence for a moment.
        ...
        if (queue_was_empty && !service_stopped_.load()) {
            underrun_count++;
            if (underrun_count % 10 == 1) {
                ESP_LOGW(TAG, "Speaker queue underrun #%d (starved of PCM)", underrun_count);
            }
        }
        if (service_stopped_.load()) {
            break;
...
int AudioService::GetOutputSampleRate() const {
    return codec_ != nullptr ? codec_->output_sample_rate() : 24000;
}

void AudioService::SetMusicPlaying(bool playing) { music_playing_.store(playing); }

bool AudioService::PushPcmToPlaybackQueue(std::vector<int16_t>&& pcm, int max_queue_depth) {
    std::unique_lock<std::mutex> lock(audio_queue_mutex_);
    if (service_stopped_.load()) {
        return false;
    }
    // Wait while the queue holds too many chunks (paced by AudioOutputTask).
    bool room =
        audio_queue_cv_.wait_for(lock, std::chrono::milliseconds(2000), [this, max_queue_depth]() {
            return service_stopped_.load() || audio_playback_queue_.size() < max_queue_depth;
        });
    if (!room || service_stopped_.load()) {
        return false;
    }
    auto task = std::make_unique<AudioTask>();
    task->type = kAudioTaskTypeDecodeToPlaybackQueue;
